  ...
  const handleLoginWithTwoFactor = async (data: LoginFormData) => {
    ...
    fetch('http://127.0.0.1:7532/ingest/1bec87db-370e-45a9-bed5-692fbf3f003b', {
      ...
      body: JSON.stringify({
        ...
        runId: 'pre-fix',
        hypothesisId: 'E',
        location: 'apps/frontend/src/features/auth/useLogin.ts:26',
        message: 'Login submit started',
        data: {
          hasEmail: !!data.email,
          emailLength: data.email ? String(data.email).length : 0,
          hasPassword: !!data.password,
          passwordLength: data.password ? String(data.password).length : 0,
        },
        timestamp: Date.now(),
      }),
    }).catch(() => {});
    ...
    fetch('http://127.0.0.1:7532/ingest/1bec87db-370e-45a9-bed5-692fbf3f003b', {
      ...
      body: JSON.stringify({
        ...
        runId: 'pre-fix',
        hypothesisId: 'E',
        location: 'apps/frontend/src/features/auth/useLogin.ts:30',
        message: 'Login submit result',
        data: {
          success: !!result?.success,
          requiresTwoFactor: !!result?.requiresTwoFactor,
          hasTempUserId: !!result?.tempUserId,
