...
export function P2POfferCard({
  ...
  const exchangeRate = (FALLBACK_EXCHANGE_RATES[offer.fiatCurrency] || 1) * (1 + Number(offer.exchangeRateMargin) / 100);

  return (
    <div className={`border rounded-lg p-4 hover:border-primary transition-colors bg-card ${className}`}>
      ...
      <div className='space-y-2 mb-4'>
        <div className='flex items-center justify-between text-sm'>
          ...
          <span className='font-medium'>
            {formatCurrency(exchangeRate, { currency: offer.fiatCurrency })}/USD (+{offer.exchangeRateMargin}%)
          </span>
        </div>
        <div className='flex items-center justify-between text-sm'>
