...
export const p2pMutations = {
  // P2P取引リクエストを作成（買い手用）
  createP2PTradeRequest: async (
    _: unknown,
    args: { input: { offerId: string; amountUsd: number } },
    ctx: {
      user?: { id: string };
      prisma: {
        p2POffer: {
          findUnique: (arg: { where: { id: string } }) => Promise<{
            sellerId: string;
            minAmountUsd: number;
            maxAmountUsd: number;
            isActive: boolean;
      ...
      authService: { hasPermission: (userId: string, permission: string) => Promise<boolean> };
    }
  ) => {
    ...
    if (amountUsd < offer.minAmountUsd || amountUsd > offer.maxAmountUsd)
      throw new Error('金額が範囲外です');

    const tradeService = new P2PTradeService(ctx.prisma as any);
    return tradeService.createTradeRequest(
      userId,
      Number(amountUsd),
      offer.fiatCurrency,
      offer.paymentMethod,
  ...
  createP2POffer: async (
    ...
    ctx: {
      user?: { id: string };
      prisma: {
        p2POffer: { create: (arg: { data: Record<string, unknown> }) => Promise<unknown> };
      };
      authService: { hasPermission: (userId: string, permission: string) => Promise<boolean> };
    }
  ) => {
    ...
    const { input } = args;
    if (input.minAmountUsd <= 0 || input.maxAmountUsd <= 0)
      throw new Error('金額は正の数である必要があります');
    if (input.minAmountUsd >= input.maxAmountUsd)
      throw new Error('最小金額は最大金額より小さくする必要があります');
  ...
  // P2Pオファーを更新（売り手用）
  updateP2POffer: async (
    _: unknown,
    args: { input: { offerId: string; [key: string]: unknown } },
    ctx: {
      user?: { id: string };
      prisma: {
        p2POffer: {
          findUnique: (arg: { where: { id: string } }) => Promise<{ sellerId: string } | null>;
          update: (arg: {
            where: { id: string };
  ...
  deleteP2POffer: async (
    ...
    ctx: {
      ...
      prisma: {
        p2POffer: {
          findUnique: (arg: { where: { id: string } }) => Promise<{ sellerId: string } | null>;
          delete: (arg: { where: { id: string } }) => Promise<void>;
        };
      };
      authService: { hasPermission: (userId: string, permission: string) => Promise<boolean> };
    }
  ...
  // P2P取引リクエストをキャンセル
  cancelP2PTradeRequest: async (
    _: unknown,
    args: { tradeId: string },
    ctx: { user?: { id: string }; prisma: any }
  ) => {
    const userId = ctx.user?.id;
    if (!userId) throw new Error('認証が必要です');
  ...
  // 支払い受領を確認（売り手用）
  confirmP2PPaymentReceived: async (
    _: unknown,
    args: { input: { tradeId: string } },
    ctx: {
      user?: { id: string };
      prisma: any;
      authService: { hasPermission: (userId: string, permission: string) => Promise<boolean> };
